...
import HypergeometricFunctions: drummond0F0, drummond1F0, drummond0F1,
                                ...
                                drummond2F1, drummondpFq, wenigerpFq
...
@testset "₀F₀" begin
    for (S, T) in ((Float16, Float32),
        ...
        for z in S(-4):S(0.25):S(1)
            ...
            @test wenigerpFq(S[], S[], z) ≈ S(mFn(T[], T[], T(z))) atol=atol rtol=rtol
        end
    end
end
...
@testset "₀F₁" begin
    for (S, T) in ((Float32, Float64),
        ...
        for α in S(-1.5):S(1.0):S(1.5), z in S(-0.75):S(0.25):S(0.75)
            ...
            @test wenigerpFq(S[], S[α], z) ≈ S(mFn(T[], T[α], T(z))) atol=atol rtol=rtol
        end
    end
end
...
@testset "₂F₀" begin
    for (S, T) in ((Float32, Float64),
        ...
        for α in S(-1.5):S(1.0):S(1.5), β in S(-1.5):S(1.0):S(1.5), z in S(-1.0):S(0.25):S(0.0)
            ...
            @test wenigerpFq(S[α, β], S[], z) ≈ S(wenigerpFq(T[α, β], T[], T(z))) atol=atol rtol=rtol
        end
    end
end
...
@testset "₀F₂" begin
    for (S, T) in ((Float32, Float64),
        ...
        for α in S(-1.5):S(1.0):S(1.5), β in S(-1.5):S(1.0):S(1.5), z in S(-0.75):S(0.25):S(0.75)
            ...
            @test wenigerpFq(S[], S[α, β], z) ≈ S(mFn(T[], T[α, β], T(z))) atol=atol rtol=rtol
        end
    end
end
...
@testset "₂F₁ with interval arithmetic" begin
    for (S, IS, T, IT) in ((Float32, Interval{Float32}, Float64, Interval{Float64}),
        ...
        β = S(0.5)
        γ = S(0.375)
        for z in S(-0.125):S(0.25):S(0.125)
            @test drummondpFq(IS[α, β], IS[γ], IS(z)) ≈ IS(mFn(IT[α, β], IT[γ], IT(z))) atol=atol rtol=rtol
            @test wenigerpFq(IS[α, β], IS[γ], IS(z)) ≈ IS(mFn(IT[α, β], IT[γ], IT(z))) atol=atol rtol=rtol
        end
    end
end
